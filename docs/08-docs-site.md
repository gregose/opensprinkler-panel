...
| `manual-run.png` | A single station running: `STATION N` eyebrow, station name, large amber countdown, **Next ›**, **Pause** and **■ Stop** buttons (the same three-button row as the program run), active pill highlighted, and live current in the top bar. | Tap a station pill from idle. Capture while the countdown is a clean value (e.g. after setting Run time to 1:00). |
| `manual-paused.png` | A manual single-station run **paused**: frozen amber countdown with the two-line **`PAUSED` / `Resumes in MM:SS`** block beside it, the middle button reading **Resume**, and the top bar status word **`Paused`** (amber). | From a manual run, tap **Pause** (or pause from the app). Capture while `Resumes in` shows a clean value. |
...
| `program-paused.png` | A program run **paused**: frozen amber countdown with the two-line **`PAUSED` / `Resumes in MM:SS`** block, the middle button reading **Resume**, the current queue row showing the ⏸ pause glyph, and the top bar status word **`Program paused`** (amber). | From a program run, tap **Pause**. Capture while `Resumes in` shows a clean value. |
