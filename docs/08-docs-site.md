...
| `manual-paused.png` | A manual single-station run **paused**: frozen amber countdown with the two-line **`PAUSED` / `Resumes in MM:SS`** block beside it, and the middle button reading **Resume**. The top bar is unchanged (no paused status word). | From a manual run, tap **Pause** (or pause from the app). Capture while `Resumes in` shows a clean value. |
